...
const Messages = () => {
  ...
  const myIdRef = useRef('')       // server truth for isMine check
  const activeRef = useRef(null)   // FIX: ref to avoid stale closure in socket
  ...
  const openConvo = async (convo) => {
    setActive(convo)
    activeRef.current = convo   // FIX: keep ref in sync so socket always sees latest
    setMessages([])
    setIsTyping(false)
    try {
  ...
  const sendMsg = async () => {
    ...
    try {
      const res = await api.post(
        `/messages/${active.user._id}`,
        { text: msgText }
      )
      const saved = res?.data?.message
      if (!saved) return
      // Sender adds via API only — socket sends to receiver only
  ...
  useEffect(() => {
    ...
    const onNewMessage = (incoming) => {
      const currentActive = activeRef.current  // FIX: read from ref not state
      if (!currentActive) { fetchConvos(); return }

      const otherId = currentActive.user._id?.toString()
      const fromId =
    ...
    const onTyping = ({ senderId }) => {
      const currentActive = activeRef.current  // FIX: read from ref
      if (currentActive &&
          senderId === currentActive.user._id?.toString()) {
        setIsTyping(true)
      }
    }
  ...
  }, [socket])  // FIX: removed 'active' from deps — ref handles it now
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
      ...
      <div className="w-[280px] flex-shrink-0 border-r
        border-[var(--border)] bg-[var(--bg-card)] flex flex-col">

        <div className="px-4 py-4 border-b border-[var(--border)]">
          <h2 className="font-bold text-[18px] text-[var(--text-main)]">
            Messages
          </h2>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center py-12 px-4">
              ...
              <p className="text-[var(--text-main)] font-semibold text-[14px]">
                No conversations
              </p>
              ...
              <button
                ...
                className="mt-3 px-4 py-2 rounded-xl text-[12px]
                  font-medium text-white bg-gradient-to-r
            ...
            conversations.map(convo => (
              <div
                ...
                className={`flex items-center gap-3 px-4 py-3
                  cursor-pointer border-l-[3px] transition
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-[12px] truncate
                    ${convo.unreadCount > 0
                      ...
                      : 'text-[var(--text-muted)]'}`}>
                    {convo.lastMessage
                      ? convo.lastMessage.sender?._id?.toString()
                          === user?._id?.toString()
                ...
                {convo.unreadCount > 0 && (
                  <span className="bg-[#6C63FF] text-white text-[10px]
        ...
        {!active ? (
          ...
          <>
            ...
            <div className="flex-shrink-0 flex items-center gap-3
              ...
              <button
                onClick={() => navigate(
                  `/profile/${active.user.username}`
                )}
                className="text-[12px] font-medium text-[#6C63FF]
                  border border-[#6C63FF44] px-4 py-1.5 rounded-full
                  hover:bg-[#6C63FF11] transition"
              >
                View Profile →
              </button>
            </div>
                  ...
                  return (
                    ...
                    >
                      <Avatar
                        ...
                        size="xs"
                      />
                      <div className={`max-w-[60%] px-4 py-2.5
                    ...
                    {[0, 1, 2].map(i => (
                      <div
                        ...
                        className="w-2 h-2 rounded-full
                          bg-[var(--text-muted)] animate-bounce"
                        style={{
            ...
            <div className="flex-shrink-0 flex items-center gap-3
              ...
              <input
                ...
                className="flex-1 min-w-0 bg-[var(--bg-input)]
                  border border-[var(--border)] rounded-full
                  px-5 py-3 text-[var(--text-main)] text-[14px]
                  focus:border-[#6C63FF] focus:ring-2
                  focus:ring-[#6C63FF22] outline-none transition"
              />
              <button
                onClick={sendMsg}
                ...
                className="flex-shrink-0 w-12 h-12 rounded-full
                  ...
                  shadow-[0_0_20px_#6C63FF55] hover:scale-105
                  hover:shadow-[0_0_28px_#6C63FF88]
